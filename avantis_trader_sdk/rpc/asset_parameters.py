...
class AssetParametersRPC:
    ...
    async def get_asset_utilization(self):
        ...
        oi_limits, aggregate_response = await asyncio.gather(oi_limits_task, aggregate_task)
        
        utilization = {}
        for pair_index, (pair_data, oi_limit) in enumerate(zip(aggregate_response[1], oi_limits.limits.values())):
    ...
    async def get_asset_skew(self):
        ...
        for pair, long_ratio in oi.longRatio.items():
            ...
            skew[pair] = long_ratio * 100 / (long_ratio + short_ratio) if long_ratio + short_ratio else 0
