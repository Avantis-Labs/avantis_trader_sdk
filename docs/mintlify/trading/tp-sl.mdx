...
| On positions | `price_triggers` entries with `is_global=True` (synthetic `global-*` ids) | `price_triggers` entries with `is_global=False` (real `documentId`) |
...
real `documentId`; the global TP/SL shows up as read-only `is_global=True`
entries with synthetic `global-tp-*` / `global-sl-*` ids. Passing a synthetic
