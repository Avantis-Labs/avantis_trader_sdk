...
| --- | --- | --- |
| `pair` | required | Symbol (`"ETH/USD"`) or pair index |
...
| `is_pnl` | `False` | Set from `Position.is_pnl` so ZFP closes route correctly |
...
| `isPnl`, `coinExposure`, `isCoinExposureFixed` | Zero-fee flag, base-asset exposure (1e10), coin-sized flag |
