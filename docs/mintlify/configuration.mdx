---
title: Configuration
description: "Environment variables, constructor overrides, networks, and signers."
---

The SDK resolves configuration in priority order: **constructor arguments > environment variables > network profile defaults**. Most users set two env vars and nothing else.

```bash
export AVANTIS_PRIVATE_KEY=0x...      # signing key (API key or wallet key)
export AVANTIS_TRADER_ADDRESS=0x...   # wallet address (only when using an API key)
```

## Environment variables

| Variable | Default | Purpose |
| --- | --- | --- |
| `AVANTIS_PRIVATE_KEY` | — | Signing key. An API/agent key from the Avantis UI, or your wallet key. |
| `AVANTIS_TRADER_ADDRESS` | — | Your wallet (holds USDC and positions). Set only when the key above is a delegate/API key. |
| `AVANTIS_NETWORK` | `testnet` | `testnet` or `mainnet`. |
| `AVANTIS_EXECUTION` | `relayer` | `relayer` (gasless, default) or `direct` (self-broadcast). |
| `AVANTIS_RPC_URL` | — | Base RPC. Required only for `direct` execution. |

Per-service endpoint overrides (rarely needed): `AVANTIS_TX_BUILDER_URL`, `AVANTIS_RELAYER_URL`, `AVANTIS_DATA_API_URL`, `AVANTIS_CORE_API_URL`, `AVANTIS_HISTORY_API_URL`, `AVANTIS_RISK_API_URL`, `AVANTIS_FEED_URL`.

## Constructor overrides

Anything the env can set, the constructor can override:

```python
from avantis_trader_sdk_v2 import AsyncAvantis

client = AsyncAvantis(
    private_key="0x...",
    trader_address="0x...",
    network="mainnet",
    execution="direct",
    rpc_url="https://mainnet.base.org",
)
```

Other useful options: `timeout_s` (default 30), `relay_poll_timeout_s` (default 60, how long `wait=True` polls the relayer), `builder_code` (optional 32-byte calldata suffix for partners).

## Sync client

`Avantis` mirrors the async surface with blocking calls — same namespaces, same methods:

```python
from avantis_trader_sdk_v2 import Avantis

client = Avantis()
client.trade.market_open("ETH/USD", "long", collateral=100, leverage=10)
client.close()
```

## Custom signers

By default the SDK builds a `LocalSigner` from `private_key`. Pass any `BaseSigner` instead — e.g. AWS KMS (install with `pip install 'avantis-trader-sdk-v2[kms]'`):

```python
from avantis_trader_sdk_v2.signing import KmsSigner

client = AsyncAvantis(signer=KmsSigner("alias/avantis-agent", region_name="us-east-1"))
```

<Note>
Streams (Socket.IO pair data) need an extra: `pip install 'avantis-trader-sdk-v2[streams]'`.
Python 3.10+ is required.
</Note>
