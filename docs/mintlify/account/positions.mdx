...
| `offchain_orders` | Deprecated: off-chain orders only; use `price_triggers` |
...
- **Global (on-chain) TP/SL** — `is_global` is `True`. These are the levels
  set at open / via `trade.update_tp_sl()`. Their `document_id` is synthetic
  (`global-tp-...` / `global-sl-...`) and is **not** accepted by the partial
  TP/SL CRUD; the SDK rejects it client-side too.
- **Partial (off-chain) triggers** — `is_global` is `False`. Signed
  [partial TP/SL](/trading/tp-sl#partial-tpsl-trigger-orders) orders whose
  `document_id` works with `trade.update_partial_tp_sl()` and
  `trade.cancel_partial_tp_sl()`.

```python
for trig in pos.price_triggers:
    print(trig.kind, float(trig.price), "global" if trig.is_global else trig.document_id)
