...
**`MarketOrderExecuted`** (terminal for opens/closes): the final fill. `t` is
...
**`PositionSizeIncreased`** (terminal for increases). Note the shape differs
...
`BatchedMarketClient.execute` / `.wait`), as sync or async callables; see
