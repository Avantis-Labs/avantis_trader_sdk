...
- **Log emitter addresses.** v2 splits the monoliths into
  ...
  `address(this)` — logs still surface on the proxy addresses you already
  subscribe to.
...
- **`PriceAggregator.fulfill`** now takes
  ...
  instead of `int spreadP` — a hard ABI break.
...
- **View mutability changed.** Some conceptual reads are non-`view` in v2
  ...
  `withinExposureLimits`, …) — fine over `eth_call`, breaks strict
  `staticcall` assumptions. Conversely `Multicall.getPositions` is now
  `view` and returns **compact arrays** (active entries only — key on
