...
| `asset_parameters.get_price_impact_spread()` / `get_skew_impact_spread()` / `get_opening_price_impact_spread()` | `markets.spread(...)` (risk engine v2; `markets.dynamic_spread(...)` on mainnet until the cutover); v2's spread model is flow-based and served by the risk API |
...
| (none) | `client.pair_data_stream()` (live pair-catalog updates), `client.order_event_stream()` (your order lifecycle events) |
