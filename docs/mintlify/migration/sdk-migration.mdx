...
The package name is unchanged (`avantis-trader-sdk` on PyPI). To stay on
the v1 SDK until migration day, pin it:

```bash
pip install "avantis-trader-sdk<2"   # v1 (until August 12)
...
export AVANTIS_NETWORK=mainnet        # default is testnet
```
...
1. **Key type.** The closest thing to 0.x is using your trader key directly
   ...
   The recommended setup is an **API key** — a delegate key created on the
   Avantis UI that can trade but never withdraw. See
...
| `trade.build_trade_open_tx(TradeInput, MARKET_ZERO_FEE, ...)` | `trade.market_open(..., zero_fee=True)` |
...
| `trade.get_trades(trader)` | `account.positions()` — includes liquidation price, rollover, funding, and open limit orders |
...
- **No `TradeInput` struct.** Orders are keyword arguments; pairs are
  symbols or indexes interchangeably (`"ETH/USD"` or `1`) — no
  `get_pair_index` round-trip needed.
...
async with AsyncAvantis() as client:
    ...
    await client.trade.market_close(
        ...
        collateral_to_close=float(pos.collateral), is_pnl=pos.is_pnl,
    )
```
...
| `asset_parameters.get_price_impact_spread()` / `get_skew_impact_spread()` / `get_opening_price_impact_spread()` | `markets.dynamic_spread(...)` — v2's spread model is flow-based and served by the risk API |
...
| — | `client.pair_data_stream()` (live pair-catalog updates), `client.order_event_stream()` (your order lifecycle events) |
