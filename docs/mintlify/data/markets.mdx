...
<Note>
The v2 spread engine is live on testnet. On mainnet the endpoint is routed but the engine is not serving yet (requests error until the v2 cutover) — use `client.markets.dynamic_spread(...)` (same arguments, response key `dynamicSpreadPct`) there in the meantime.
</Note>
