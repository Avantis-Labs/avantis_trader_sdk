...
`avantis_trader_sdk_v2.compute` reimplements the Avantis app's math as pure functions: no I/O, human units in and out. Numbers match what the UI displays.

```python
from avantis_trader_sdk_v2 import compute
```
...
| `net_pnl(...)` | Full breakdown with fees, funding, loss protection, ZFP profit share |
| `position_net_pnl(position, pair_info, current_price)` | Convenience wrapper over a fetched `Position` |
| `pnl_type_fee`, `pnl_fee_by_gross_profit_p`, `adjusted_max_gain_p` | ZFP tiered profit-share math |
...
`pnl_order_min_sl(leverage)` gives the minimum SL percent for zero-fee positions.
