...
- **Golden vectors** — all 15 intent kinds are tested against signature vectors produced by the on-chain hashing library itself, so local encoding provably matches the contracts.
...
1. Generate an API key in the Avantis UI; never put your wallet key on a server.
