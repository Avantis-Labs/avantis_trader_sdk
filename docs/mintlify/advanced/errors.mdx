...
Every failure raises a typed exception from `avantis_trader_sdk_v2.errors`, all descending from `AvantisError`.

```python
from avantis_trader_sdk_v2.errors import ValidationError, RelayTimeoutError
...
| `RelayError` | Relayer rejected or failed the request (`.request_id`) | Inspect message |
...
<Warning>
`DigestMismatchError` means the SDK's local encoding disagrees with the API — encoding drift or a tampered response. Nothing was signed or submitted. Do not work around it; upgrade the SDK or report the issue.
</Warning>
